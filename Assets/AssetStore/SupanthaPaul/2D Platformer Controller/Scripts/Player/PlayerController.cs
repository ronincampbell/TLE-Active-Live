...
namespace SupanthaPaul
{
	public class PlayerController : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if (!isDashing && !m_hasDashedInAir && m_dashCooldown <= 0f && canMove)
			{
			...
			// Jumping
			if(InputSystem.Jump() && m_extraJumps > 0 && !isGrounded && !m_wallGrabbing && canMove)	// extra jumping
			{
			...
			else if(InputSystem.Jump() && (isGrounded || m_groundedRemember > 0f) && canMove)	// normal single jumping
			{
			...
			if (isCurrentlyPlayable && !m_wallGrabbing && isGrounded && !isDashing && canMove)
			{
				// Melee attack
		...
		{
			// Create a circle that detects enemies
			Collider2D[] hitEnemies = Physics2D.OverlapCircleAll(attackPoint.transform.position, attackRange, whatIsEnemies);
			if (hitEnemies.Length == 0) 
			{
				Debug.LogWarning("No enemies found in range.");
				return;
			}
			// Apply damage to all enemies detected
			foreach (Collider2D enemy in hitEnemies)
			{
				if (enemy == null) 
				{
					Debug.LogWarning("Found a null enemy in hitEnemies.");
					continue;
				}
